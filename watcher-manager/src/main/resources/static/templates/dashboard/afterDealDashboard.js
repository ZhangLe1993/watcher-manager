Template.afterDealDashboard.rendered = function () {
    $('.navi-tab').removeClass('active');
    $('#dashboard').addClass('active');
    $('#tradeDealStats').addClass('active');
    ...
    requestURL(dataService + "/live/order", {}).done(function (data) {
        ...
        var storeData = publicHandler(data, 'area', 'current_success_num', 'last_success_num', "method", "门店订单");
        drawPublicChartNoScroll(storeData, 'storeNumChart', '提交订单量', true, "num", "method", "门店订单");

        var indoorData = publicHandler(data, 'area', 'current_success_num', 'last_success_num', "method", "上门订单");
        ...
        var indoorAData = publicHandler(data, 'area', 'current_success_amount', 'last_success_amount', "method", "上门订单");
        drawPublicChartNoScroll(indoorAData, 'indoorAmountChart', '提交订单额', true, "amount", "method", "上门订单");

        var storeAData = publicHandler(data, 'area', 'current_success_amount', 'last_success_amount', "method", "门店订单");
...
function customWarnModal(modelSelector, titleSelector, area, defaults, nextFilterField, nextFilterTarget) {
    $("#" + modelSelector).off('shown.bs.modal').on('shown.bs.modal', function () {
        ...
        var showField = (defaults == 'num') ? 'current_success_num' : 'current_success_amount';
        var subShowField = (defaults == 'num') ? 'last_success_num' : 'last_success_amount';
