Template.outletsMoneyReport.rendered = function () {
...
function drawOutletsTradeReport(tradeNumDataSet) {
    /*var yesterdayStats = outletsTradeReport.findOne({}, {sort: {date: -1}});
     var days2Before = new Date(new Date(yesterdayStats.date).getTime() - 24 * 3600 * 1000).format("yyyy-MM-dd");
     var days2BeforeStats = outletsTradeReport.findOne({date: days2Before});
     var rate1 = (yesterdayStats.total - days2BeforeStats.total) / days2BeforeStats.total;
     var percent1 = Math.round(rate1 * 10000) / 100;
     var days7Before = new Date(new Date(yesterdayStats.date).getTime() - 7 * 24 * 3600 * 1000).format("yyyy-MM-dd");
     var days7BeforeStats = outletsTradeReport.findOne({date: days7Before});
     var rate7 = (yesterdayStats.total - days7BeforeStats.total) / days7BeforeStats.total;
     ...
     var tradeNumDataSet = outletsTradeReport.find({$and: [{date: {$gte: startDate}}, {date: {$lte: endDate}}]},{sort:{date:1}}).fetch();*/
    ...
    var outletsTradeReportChart = echarts.init(document.getElementById('outletsMoneyReport'));
