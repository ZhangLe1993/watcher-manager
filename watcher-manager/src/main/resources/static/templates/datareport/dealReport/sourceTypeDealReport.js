Template.sourceTypeDealReport.rendered = function () {
    ...
    requestURL(dataService+ "/tradeCountDetail/sourceType", query).done(function (data) {
           drawSourceTypeDealReport(data,flag);
        });
...
function drawSourceTypeDealReport(data,flag){
    ...
    var sourceTypeDealReportChart = echarts.init(document.getElementById('sourceTypeDealLineReport'));
    sourceTypeDealReportChart.clear()
    sourceTypeDealReportChart.setOption(option)
    window.addEventListener('resize',sourceTypeDealReportChart.resize)
}
