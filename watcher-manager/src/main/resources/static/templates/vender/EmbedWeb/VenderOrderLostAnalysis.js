 ...
 */
Template.VenderOrderLostAnalysis.rendered = function () {
    $('.navi-tab').removeClass('active');
    $('#VenderTab').addClass('active');
    $('#VenderOrderLostAnalysis').addClass('active');
    ...
    requestURL(dataService+"/Vender/getAgentAreaFilterOptions",query).done(function(data){
        ...
        renderOptions(".province",data.groupProvince)

        $(".city").attr("multiple","multiple");
        renderOptions(".city",data.groupCity)

        $(".district").attr("multiple","multiple");
        renderOptions(".district",data.groupDistrict)

        $(".vender").attr("multiple","multiple");
        renderOptions(".vender",data.vender)
...
function getAggregateWebTrafficData(filter){
    ...
    var dfd = $.Deferred();
    requestURL(dataService+"/Vender/getAgentOrderLostAnalysis",query).done(function(ret){
        dfd.resolve(ret)
    });
    return dfd.promise()
}
...
function renderTable(tableName,data) {
    data.forEach(function(e){
        e.differCnt= e.inqueryCnt- e.submitCnt;
        e.differNum= e.inqueryNum- e.submitNum;
    })
    ...
    var fixRow = '"<tr style="background-color: #acc087;"", " ", "", "", " data-index="0"", "", "", ">", "<td  style=""    >总计</td>", "<td  style=""    >-</td>","<td  style=""    >-</td>","<td  style=""    >-</td>","<td  style=""    >-</td>", "<td  style=""    >'+inqueryCnt+'</td>","<td style="">'+inqueryNum+'</td>", "<td  style=""    >'+submitCnt+'</td>", "<td  style=""    >'+submitNum+'</td>", "<td  style=""    >'+diffCnt+'</td>", "<td  style=""    >'+diffNum+'</td>"'
